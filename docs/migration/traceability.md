...
| CKU-EDIT | `ck-edit` | edit, format, search, save, resolve conflict, and close Markdown documents | WP-8 | native injected-file editor with open/save/save-as, explicit dirty Save/Discard/Cancel close, and application-owned Markdown syntax profile implemented; Markdown transformations and richer external-conflict resolution remain |
| CKU-CHAT | `ck-chat` | edit/send/cancel prompts, stream/copy/export responses, manage models/prompts | WP-9 | native FlowView transcript, prompt dialog, injected streaming response service, cancellation, worker adapter, UI-thread lifetime guard, shared Markdown-to-FlowView adaptation, and injected transcript export implemented. Prompt selection/create/edit/default restoration/confirmed custom deletion and downloaded-model selection/deactivation/confirmed local deletion use framework-neutral adapters over `ckai_core`, with active prompt and model IDs propagated into response requests; model download/loading and richer progress remain |
