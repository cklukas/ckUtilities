...
| CKU-CONFIG | `ck-config` | inspect/edit/reset/import/export configuration and key bindings | WP-7 | native injected-registry inspector/editor with typed edit/reset and injected JSON-default save/reload/import/export implemented. Its shortcut table now captures typed normalized chords, reports occupied bindings before an explicit replacement, and persists by stable command key. One suite catalog now drives both each executable's declared title/category/default chord and Config's command-only registries, so app-specific bindings are edited and stored under the target app while shared `ckv.*` bindings remain global; target executables reload both at startup. Scheme selection and import rollback remain |
