...
| CKU-LAUNCH | `ck-utilities` | launch tool, multi-window utilities, calculator, ASCII, color, event diagnostics | WP-4 | native multi-window launcher, calendar, ASCII table, calculator, and bounded application diagnostics implemented; color remains |
...
| CV-GAP-004 | Reusable color selection may be a useful generic control. Bounded application diagnostics use ckVision's existing injected diagnostics observer; no raw-event observer is required. | CKU-LAUNCH | color-selection investigation remains open |
