...
| CKU-CHAT | `ck-chat` | edit/send/cancel prompts, stream/copy/export responses, manage models/prompts | WP-9 | native FlowView transcript, prompt dialog, injected streaming response service, cancellation, worker adapter, UI-thread lifetime guard, shared Markdown-to-FlowView adaptation, and injected transcript export implemented. Prompt selection, create/edit, default restoration, confirmed custom deletion, and propagation into response requests use a framework-neutral adapter over `ckai_core`; model management and richer progress remain |
