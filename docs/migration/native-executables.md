...
| Edit | `ck-edit-ckvision` | native document open/save/save-as, dirty Save/Discard/Cancel close, Markdown syntax profile, transactional whitespace normalization, bold/italic/strikethrough/delimiter-safe inline-code selection toggles, heading 1–6 toggles that protect code, task-list state, block-quote level, basic bullet/ordered-list toggles, destination-prompted link insertion/removal, and conservative 80-code-point paragraph reflow, Unicode selection restoration, and explicit external-save-conflict recovery | deeper list structure/smart-list/find-replace transformations, final acceptance |
