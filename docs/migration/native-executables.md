...
| Chat | `ck-chat-ckvision` | FlowView transcript, Markdown adaptation, streaming/cancel/copy/export, injected worker response and transcript services, prompt and model lifecycle, and cancellable background downloads with rate-limited UI-thread progress backed by `ckai_core` | model runtime loading, parity acceptance |
