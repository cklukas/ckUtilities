...
| Config | `ck-config-ckvision` | injected option registry inspect/edit/reset/save/reload/import/export plus a keyboard-only normalized shortcut editor with explicit collision replacement and suite-wide target-application command catalog | scheme selection/import rollback, final acceptance |
