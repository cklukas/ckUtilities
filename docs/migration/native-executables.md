...
| Config | `ck-config-ckvision` | injected option registry inspect/edit/reset/save/reload/import/export plus a keyboard-only normalized shortcut editor with explicit collision replacement | suite-wide catalog for other applications' command bindings, scheme selection/import rollback, final acceptance |
