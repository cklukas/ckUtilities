...
- WP-6: `ck-du-ckvision` maps application-owned directory snapshots through a
  ...
  non-iCloud path before issuing a provider request. The accepted provider
  policy now revalidates the scan-root-contained selected directory without
  following symbolic links, rejects eviction until iCloud reports upload, and
  reports a cancellation arriving after request acceptance honestly. Other
  providers remain unsupported. Rescan now keeps the last valid snapshot visible until a
...
- WP-7: `ck-config-ckvision` accepts an injected option registry and exposes
  ...
  starts, and selection or deletion commands write the resulting stable IDs
  back through an injected profile policy. A failed save restores the
  registry's prior in-memory values rather than leaving it stale.
...
- WP-8: `ck-edit-ckvision` uses ckVision's EditorDocument, EditorWindow, and
  ...
  Quote commands add or remove a selection-wide quote level while preserving
  blank-line continuity and protected code. Basic bullet and ordered-list
  commands convert ordinary selected lines, number ordered selections
  sequentially, preserve task markers while changing list styles, and remove
  task markers when converting a task list to plain text. Link and image
  commands use native destination dialogs to insert or remove a selected
  complete link/image or its label/alt text; parentheses in destinations are
  supported. The footnote command accepts a safe identifier and uses one
  multi-edit document transaction to add a reference plus an appended,
  blank-line-separated definition, placing the cursor in that definition body.
  Native literal
...
- `CKTOOLS_CKVISION_CUTOVER=ON` rehearses the release composition against the
  ...
  quit chord, then require clean exit and restored persistent terminal mode.
